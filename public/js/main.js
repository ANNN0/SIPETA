...
; (function ($) {
  ...
  var menuleft = function () {
    if ($('div').hasClass('section-menu-left')) {
      ...
      // Mark menus that have server-side active class on page load
      bt.each(function() {
        if ($(this).hasClass('active')) {
          $(this).attr('data-server-active', 'true');
          // Ensure sub-menu is visible
      ...
      bt.on("click", function () {
        ...
        if (clickedMenu.hasClass("active")) {
          // Only allow closing if NOT server-active
          if (!clickedMenu.attr('data-server-active')) {
            clickedMenu.children(".sub-menu").slideUp(args);
            clickedMenu.removeClass("active");
          }
        } else {
          // Close other menus (except server-active ones)
          $(".has-children").not(clickedMenu).each(function() {
            if (!$(this).attr('data-server-active')) {
          ...
          // Open clicked menu
          clickedMenu.children(".sub-menu").slideDown(args);
